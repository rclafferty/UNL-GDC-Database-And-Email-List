...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([["Fall2019Attendance.txt",new URL("./files/Fall2019Attendance.txt",import.meta.url)]]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer()).define(["md"], function(md){return(
md`# Attendance From Differet Years
### Fall 2019
`
)});
...
d3.tsvParse(await FileAttachment("Fall2019Attendance.txt").text(), d => (d3.autoType(d).date = new Date(d.date), d))
)});
